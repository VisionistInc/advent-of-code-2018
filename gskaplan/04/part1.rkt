...
(define sleepiest-guard
  (first
   (argmax
    second
    (hash-map minutes-by-guard (lambda (k v) (list k (length v))))
    )))

(define sleepiest-guard-minutes-counted
  (foldl
   (lambda (m current)
     (hash-update current m (lambda (x) (+ 1 x)) 0)
     )
   (make-immutable-hash)
   (hash-ref minutes-by-guard sleepiest-guard)
   ))

(define sleepiest-guard-sleepiest-minute
  (first
   (argmax
    second
    (hash-map sleepiest-guard-minutes-counted (lambda (k v) (list k v)))
    )))

(* sleepiest-guard sleepiest-guard-sleepiest-minute)
