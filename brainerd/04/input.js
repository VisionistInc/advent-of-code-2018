var input = `[1518-09-16 23:57] Guard #1889 begins shift
...
let eventMap = inputArray.map(thing => {
  let date = thing.substr(1, 16);
  var event;
  switch (thing.substr(19,1)) {
    case "G": 
      event = {
        shift_start: true,
        id: thing.split(" ")[3].substr(1)
      }
      break;
    case "f":
      event = {
...
var beginningDate = eventMap[0].time;
var endDate = eventMap[eventMap.length - 1].time;
var msBetween = endDate - beginningDate;
var secondsBetween = msBetween / 1000;
var minutesBetween = Math.ceil(secondsBetween / 60);
var allTheMinutes = Array(minutesBetween);

for (var i = 0;i<minutesBetween;i++) {
  allTheMinutes[i] = {
    date: beginningDate,
    guard_status: null
  }
  beginningDate = new Date(new Date(beginningDate).setTime(new Date(beginningDate).getTime() + 1000 * 60));
}
var cursor = 0;
var tempThing ;
...
var allTheMinutes2 = []
for (var i = 0;i<minutesBetween;i++) {  
  if (allTheMinutes[i].date.toString() == eventMap[cursor].time.toString()) {
    if (eventMap[cursor].event.shift_start) {
      allTheMinutes[i].guard_status = {
        id: eventMap[cursor].event.id,
        awake: true
      }
    } else if (eventMap[cursor].event.falls_asleep) {
      allTheMinutes[i].guard_status = {
        id: allTheMinutes[i-1].guard_status.id,
        awake: false
      }
    } else if (eventMap[cursor].event.wakes_up) {
      allTheMinutes[i].guard_status = {
        id: allTheMinutes[i-1].guard_status.id,
        awake: true
      }
    ...
    allTheMinutes[i].guard_status = allTheMinutes[i-1].guard_status;
  }
}
const asleepMinutes = allTheMinutes.filter(a => a.guard_status.awake == true);

const awakepMinutes = allTheMinutes.filter(a => a.guard_status.awake == false);

module.exports = allTheMinutes;